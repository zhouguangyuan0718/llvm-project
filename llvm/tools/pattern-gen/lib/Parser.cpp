...
/// Mutable state for one CoreDSL parsing invocation.
///
/// The original parser kept this state in file-static variables.  Besides
/// making the parser non-reentrant, that allowed configuration from one input
/// to survive into the next input in the same process.  Keep the grammar and
/// IR-building helpers unchanged for now, but make their state explicitly
/// owned by a parser invocation.
struct CoreDSLParserState {
  llvm::DenseMap<uint32_t, llvm::SmallVector<Variable>> variables;
  int scopeDepth = 0;
  llvm::BasicBlock *entry = nullptr;
  CDSLInstr *curInstr = nullptr;
  int xlen = 0;
  bool noExtend = false;
  llvm::Type *regT = nullptr;
  ...
  void createVarScope() { ++scopeDepth; }
...
using ParserContext = CoreDSLParserState;

static thread_local ParserContext *ActiveParserContext = nullptr;

static ParserContext &context() {
  assert(ActiveParserContext && "parser context must be active");
  return *ActiveParserContext;
}

class ScopedParserContext {
  ParserContext *Previous = nullptr;

public:
  explicit ScopedParserContext(ParserContext &Current)
      : Previous(ActiveParserContext) {
    ActiveParserContext = &Current;
  }

  ~ScopedParserContext() { ActiveParserContext = Previous; }
};

#define variables (context().variables)
#define scopeDepth (context().scopeDepth)
#define entry (context().entry)
#define curInstr (context().curInstr)
#define xlen (context().xlen)
#define NoExtend_ (context().noExtend)
#define regT (context().regT)

static void reset_globals() { context().resetInstructionState(); }

static void create_var_scope() { context().createVarScope(); }

static void pop_var_scope() { context().popVarScope(); }
...
int ParseOperandAttributes(TokenStream &ts) { return ParseAttributes(ts); }
...
CoreDSLParser::CoreDSLParser(TokenStream &ts, bool is64Bit,
                             llvm::Module &module, bool noExtend)
    : TS(ts), Is64Bit(is64Bit), Module(module), NoExtend(noExtend),
      State(std::make_unique<CoreDSLParserState>()) {}

CoreDSLParser::~CoreDSLParser() = default;

std::vector<CDSLInstr> CoreDSLParser::parse() {
  ScopedParserContext scopedContext(*State);
  TokenStream &ts = TS;
...
std::vector<CDSLInstr> ParseCoreDSL2(TokenStream &ts, bool is64Bit,
  ...
  return CoreDSLParser(ts, is64Bit, *mod, NoExtend).parse();
}
