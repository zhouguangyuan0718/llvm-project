...
## 7. C++ API 示例：构造并打印 memref intrinsic 调用

下面这段最小示例用 LLVM 的 IRBuilder API 完成：

1. 创建模块和函数；
2. 通过 `Intrinsic::getOrInsertDeclaration` 获取 `llvm.memref.elem.add.rank2` 声明；
3. 以 rank-2 的 memref descriptor 结构体值作为参数发起调用；
4. 打印 IR。
...
int main() {
  ...
  // 声明 intrinsic: llvm.memref.elem.add.rank2
