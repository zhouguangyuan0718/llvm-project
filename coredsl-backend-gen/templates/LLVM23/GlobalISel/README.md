...
- floating-point constants and operations: `G_FCONSTANT`; operation-supported
  native scalar forms of `G_FADD`, `G_FSUB`, `G_FMUL`, `G_FDIV`, `G_FREM`,
  `G_FMA`,
...
same carrier as the legalized inputs: with native integers `[16, 32]`, both
...
`G_BRCOND` accepts every native integer carrier directly. A condition whose
...
conditions and integer conditions wider than every native carrier fail closed.
